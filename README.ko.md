...
> 💡 TodoList에 필터 컨트롤이 추가됨. 상태별 필터링 및 마감일 기준 정렬이 가능해짐.
> fetchTasks 함수가 filter, sortOrder 파라미터를 받도록 변경됨.
...
- **함수 단위 요약** — 파일 전체가 아닌 함수/컴포넌트별로 변경 내용 정리 (추가 / 제거 / 변경 상태 표시)
- **Import 변화** — 파일 단위로 추가·제거된 import를 감지
- **파라미터 변화** — 함수 파라미터의 추가·제거를 감지
- **변수 추가** — 단순 변수 할당을 가장 가까운 함수에 인라인으로 표시
- **동작 변화** — 훅 호출, setState, async/await, 조건문, 에러 처리, 반환값 감지
- **UI 변화** — 추가·제거된 JSX 컴포넌트 감지 (`div`, `span` 등 제네릭 태그 제외)
- **Props 변화** — TypeScript 인터페이스/타입 변경 감지 (긴 문자열 값은 `'...'`로 축약)
- **JSX 시맨틱 패턴** — `🔄 list → <Component>` (map), `⚡ cond && <Component>` (조건부 렌더링)
- **Gemini AI 요약** (선택) — 복잡한 훅도 1~3줄로 자연어 요약 (`> 💡 ...`)
...
| `변수 추가: x, y` | 단순 변수 할당을 인라인으로 축약 표시 |

### 섹션 종류

| 섹션 | 표시 내용 |
| --- | --- |
| **Import 변화** | 파일 단위로 추가·제거된 `import` 구문 |
| **파라미터 변화** | 추가·제거된 함수 파라미터 |
| **Props 변화** | TypeScript 인터페이스/타입 멤버 변경 (20자 초과 값은 `'...'`) |
| **동작 변화** | 훅 호출, setState, async/await, 조건문, catch 블록, 반환값 |
| **UI 변화** | 추가·제거된 의미 있는 JSX 컴포넌트; map(`🔄`) 및 조건부 렌더링(`⚡`) 패턴 |
